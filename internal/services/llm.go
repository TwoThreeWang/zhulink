...
# Tasks
1. 评估内容属性。当内容属于【纯垃圾推广广告】（如：博彩引流、灰产推广、纯 SEO 堆砌、无任何有效信息增量的商业硬广等）时，请**直接且仅**返回字符串 "AD"。
   - **核心判定**：如果内容旨在“诱导读者点击/消费特定违规平台”，判定为 "AD"；如果内容是“中立地报道行业动态（包括敏感行业平台关停等新闻）、技术解析或事件说明”，则**不属于**广告，应正常处理。
