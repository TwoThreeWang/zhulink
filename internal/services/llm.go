...
✅ 你的风格："很多人觉得 OAuth 逻辑绕，其实它就是拿『临时令牌』换『长久令牌』的过程。今天把这套流程彻底捋一遍..."
...
func buildSEOPrompt(title, content string) string {
	...
	return fmt.Sprintf(`
# Role
你是 ZhuLink 的搜索结果编辑，不是营销文案写手。你的任务是给搜索引擎和读者提供准确、克制、页面专属的元信息。
...
## Description 要求:
1. 70-120 个中文字符，最多不超过 150 个中文字符。
2. 像人工编辑写给搜索结果的一句话：准确说明这页具体讲什么，不要替页面吹嘘价值。
3. 优先使用标题和正文里真实出现的专有名词、产品名、技术名词、事件名；不要编造正文没有的信息。
4. 可以自然提到作者视角、推荐理由、问题背景、实现方法、讨论对象，但必须来自正文可见内容。
5. 内容很短或只是外链推荐时，不要硬凑“深度分析/完整指南/全面解读”，只做朴素描述。
6. 不要包含 emoji 或特殊符号。
7. 禁止 AI 味和营销味表达，包括但不限于：
   - "本文介绍了"、"本文深入探讨"、"本文全面解析"、"本文将带你了解"
   - "帮助用户"、"为读者提供"、"具有重要意义"、"值得关注"、"不容错过"
   - "核心价值"、"关键亮点"、"深度剖析"、"全面了解"、"一文看懂"
   - "通过...实现..."、"显著提升"、"赋能"、"助力"、"生态"
8. 不要堆关键词，不要写成标题党，不要使用问句或感叹句。
9. 每个 description 必须页面专属。不要输出可套用到任何技术文章的空话。

## Description 风格示例:
❌ 差: "本文深入分析了 OAuth 协议的核心原理和实践价值，帮助开发者全面了解认证授权机制。"
✅ 好: "梳理 OAuth 临时令牌、回调地址和授权码交换流程，重点说明实际接入时容易踩坑的配置。"
❌ 差: "本文介绍了一款高效工具，能够显著提升用户的工作效率，值得开发者关注。"
✅ 好: "分享一款本地身份证扫描工具，重点说明离线处理、打印尺寸和隐私数据不上传的实现取舍。"
