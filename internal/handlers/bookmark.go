...
func (h *BookmarkHandler) Toggle(c *gin.Context) {
	...
	if isBookmarked {
		c.String(http.StatusOK, fmt.Sprintf(`
			...
			<span>收藏 %d</span>`, count))
	} else {
		c.String(http.StatusOK, fmt.Sprintf(`
			<i data-lucide="bookmark" class="w-4 h-4 text-stone-400 group-hover:text-amber-500 transition-colors"></i>
			<span>收藏 %d</span>`, count))
	}
}
